...
read the shipped `theme.css` header, not this paragraph.** A flat declaration _above_ a mixed
...
(`aec38ac`): the floor it stated was _below_ Angular's own, so every build printed "unsupported
...
| 2   | ~~`gog-button` pressed state~~ **done, 21.9.0**                        | Shipped as the `:active` colour, per variant, plus the eight other pressable surfaces. The `aria-pressed` toggle _look_ shipped in the same release — an inset ring, `--gog-button-<variant>-toggled-shadow` — so it is closed too; what is still open is the same gap on `gog-chip`, see `docs/backlog.md`                                                                                                                  | S–M  |
...
| 5   | ~~Whole-row click when `gog-table` selection is on~~ **done, 21.15.0** | Shipped as a new input, `selectOnRowClick`, rather than a change to `interactiveRows` — which is what dissolved the conflict: `gogRowClick` still fires, and a table whose rows navigate keeps its behaviour. It implies interactive rows, so it cannot ship mouse-only. Building it found a real defect beside it: Space on a row's checkbox in an `interactiveRows` table never ticked the box (fixed in the same release) | M    |

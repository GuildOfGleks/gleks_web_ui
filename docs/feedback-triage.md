...
| 4   | Theme generator missing component examples                                                 | **done** | **Fixed 2026-08-31.** Six components had no tile in the gallery and no entry in the "fine-tune a single component" dropdown: Card, Collapsible, Menu, Panel, Ripple, Scroll — all present in the sidebar's 34, none in `generator-catalog.ts`'s 27. Added all six (`--gog-card-*`/`--gog-collapsible-*`/`--gog-menu-*`/`--gog-panel-*`/`--gog-ripple-*`/`--gog-scroll-*`), alphabetically, matching every existing tile's minimal-markup pattern — Menu and Ripple need a click to show anything, the same way the existing Dialog tile does. Verified live: all 33 tiles render themed, Menu's portal opens with the current palette, Scroll's thumb is visible via `[autoHide]="false"`. |
| 5   | "Full technical comparison" is stale                                                       | **partial (2026-08-31)** | Targeted fix, not a re-measurement: `compare-full.md`'s headline claim — "154 deprecated CSS custom properties" — went from stale to flatly wrong once 21.7.0 shipped and removed all 154 (`GOG_DEPRECATIONS` is now `[]`, confirmed against the installed 21.7.2). Every deprecation-count cell and the surrounding prose now say so, past tense, with a note that these are type-definition facts checked directly against 21.7.2 — unlike the page's byte/bundle figures, still from the 21.6.1 pass and now measuring against versions Material and PrimeNG have since moved past (22.1.2→22.1.4, 22.0.0→22.1.0). **Still open:** the actual bundle-size re-measurement (real `npm install` in 3 isolated folders + `esbuild` + gzip, per the page's own recipe) — deferred, real network/time cost, user declined for this pass. |
...
| 7   | Full-width examples need a dashed outline to show the container                            | **done** | **Fixed 2026-08-31.** `paginator-doc-page` already had the pattern (`border: 1px dashed var(--gog-border-color)` on its comparison box) — the other nine pages with a narrow `.width-demo` wrapper (button, checkbox, chip, input field, multiselect, select, slider, tag, textarea) didn't. `.width-demo` is duplicated per page rather than shared (no `_doc-page.scss` copy exists), so the border+padding was added to each of the nine local copies individually. Pages where `fullWidth` only shows up in an API table, or where the demo already fills its natural container (toggle's settings-list row, table's `fullWidth=false` shrink demo), needed nothing — there was no hidden boundary to reveal. Verified live on two pages. |

...
21.9.0's; see the two rows above. **The live work is `docs/lab-after-publish.md`, 13 items under
"After 21.9.0"** — no longer blocked, and one of them (`theme-starter.css`) is a check that is red
right now, because it is generated from the published package and 21.9.0 added tokens it does not
carry. 21.8.0's three lab entries were closed the day it was published, which also let the lab's
...
each entry as it lands. **It holds 13 items as of 2026-09-04, all of them 21.9.0's**; it was empty on 2026-09-03 — every section is checked off and

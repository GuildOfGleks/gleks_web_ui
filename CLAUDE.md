...
- `docs/release-21.7.0.md` — **the close-out checklist for the release currently in progress.**
  ...
  user-only step (the version bump), two gaps worth closing first (`README.md` never names the
  three presets this release added; those presets have no live `ui-showcase` surface), one
  decision that gates a CI step but blocks nothing (the contrast findings), and the lab work that
  cannot start until publish. Read it before doing anything else on 21.7.0. **Delete it once

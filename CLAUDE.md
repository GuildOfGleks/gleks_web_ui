...
**Latest release: 21.13.0 (2026-09-13), on npm and installed at the root** —
`package-lock.json` resolves `@guildofgleks/ui@21.13.0`, so `gleks-ui-lab` builds against it.
...
**`docs/lab-after-publish.md` is empty, and the lab is caught up through 21.13.0** (2026-09-13).
...
**Every check is green on `master` except `check:release`** (21.14.0 is `planned`, above).
`check:theme-starter` compares `theme.css` from the _installed_ package against the lab's mirror,
so a published release makes it fail until the lab regenerates it — expected, not a regression.
Verified 2026-09-13 along with `lint` and `build:lab`.

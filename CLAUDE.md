...
**Latest release: 21.12.0 (2026-09-10), on npm and installed at the root** —
`package-lock.json` resolves `@guildofgleks/ui@21.12.0`, so `gleks-ui-lab` builds against it.
...
**`docs/lab-after-publish.md` is empty, and the lab is caught up through 21.12.0** (2026-09-11).
Both the 21.11.0 and 21.12.0 sections were worked through and deleted, which is the state that
file is supposed to return to after each release is documented. The pass covered: the token
reference (a new Foundation — Elevation section, `--gog-control-boundary-color`, the focus-ring
colours, the four select/multiselect panel tokens, the ten-step spacing scale, `--gog-text-2xs`
and the shared field leading); the Theming page (the ladder as a system, a live six-tile demo of
it, and a "colour that is checked rather than eyeballed" section); the theme generator (Elevation
as its own foundation group, and a randomizer that picks a _style_ rather than jittering ten
numbers apart); `theme-starter.css`; four component pages; and the comparison page's accessibility
claim. **The lab is no longer deliberately deferred — the deferral ended here.**

**Every check is green on `master`, including `check:theme-starter`**, which had been the one red
one and correctly so: it compares `theme.css` from the _installed_ package against the lab's
hand-maintained mirror, so a published release makes it fail until the lab catches up. Verified
2026-09-11 along with `lint`, `test:lib` (1116 passing) and `build:lab`.

...
**Latest release: 21.14.0 (2026-09-13), on npm and installed at the root** —
`package-lock.json` resolves `@guildofgleks/ui@21.14.0`, so `gleks-ui-lab` builds against it.
...
**21.13.0 was large**: `gog-alert`, `--gog-button-focus-ring-color`, the toast's severity colours,
`check:tokens` rules J and K, **virtualization across all four collection components** —
`virtualize` on `gog-select`, `gog-multiselect`, `gog-autocomplete` and `gog-table` — and **the
first secondary entry points** (`docs/entry-points.md` phase 1): `@guildofgleks/ui/shared`
(internal, so `GOG_CONFIG` exists once), and `/table`, `/datepicker`, `/dialog` as thin re-exports
with the root's copies deprecated for 21.14.0. **Shared code lives in `projects/gleks/ui/shared/`,
not `src/lib/shared/`, and is imported as `@guildofgleks/ui/shared`** — `check:layering` fails on a
...
**21.14.0 is released** and carried phase 2 of `docs/entry-points.md`: `gog-table`,
...
root component (`docs/backlog.md`, Structural) — **and the split stops there, by decision on
2026-09-13**: at best a quarter of a four-route app's initial transfer, not worth another import break. **21.15.0 is open, heading `planned`** — `gog-table`'s
...
| 21.11.0 | **released (2026-09-09)** | The geometry release. **`--gog-space-*` is ten steps, all multiples of 4** — `-2`, `-6`, `-10`, `-14` and `-18` removed once their 102 readers moved, `-40` added because the optical ratio needs it; a consumer reading one of the five replaces it with the neighbouring step, always rounding **up**. Under that, the sweep: 164 findings across 66 components to zero in 25 commits, laws 1, 3 and 5 (the 4px grid, horizontal padding exactly twice vertical on every control, 24x24 of pointer target), and `check:geometry` became a CI step the day it reached zero. Controls got wider — the button and the whole field tier run `8 / 16 / 24 / 32 / 40` of horizontal padding. **Then two more laws, and both reversed on contact with the evidence.** L7 (icon centring) was planned against the ink's centre of mass; measuring all 41 glyphs showed the boxes already centred to a hundredth of a unit while the mass runs 3.47 units out — and correctly so, since a monoline set reads by extent. It is gated on the extent, with an area branch for a filled mark. L6 (optical area, 1.128) was _decided_ at D2 and turned out to have no instance: every candidate failed on "the square it is meant to match", the chip's avatar being a photograph. **Law 2 (concentric radii) is gated too** — a declared parent per radius, 46 of them, and the check had to resolve at two densities before it was true. L11 and L8 shipped as documentation, which is what their verdicts asked for. **Law 4 and D8 landed last, and all five laws are now gated** — six roles named against what the library already did, and the function came out as `leading = step(role)` with no size term, because 17 of 19 existing line-heights were already one value across all five sizes. That check is the first to read the _component stylesheets_ as well as `theme.css`, which is where its reach was hiding: eight literal line-heights lived there, invisible to a token-level sweep, and 35 tokens added under one of its rules were inert until another rule noticed nothing read them. Also `--gog-text-2xs`, ~45 leading tokens, and `gog-accordion`'s chevron as a ratio rather than a px ladder — which surfaced a glyph that had been overflowing its own box at every size. **D7 landed last (2026-09-09):** `gog-tooltip`, `gog-menu` and `gog-toast` cap themselves against the viewport (`min(<cap>, calc(100vw - <margin> * 2))`). The confirmation dialog was in that list until review and came out of it: it renders inside a panel already capped at `90vw` less 40px of body padding, so a clamp on the child could never bind — **an overlay nested in another overlay inherits its viewport cap**, which is the general rule that finding produced, and three of the four move from `px` to `ch` — a real bug in the fix itself, caught live rather than by either check: `ch` resolves against the font of the element carrying `max-width`, not a descendant's, which toast's own message/container split got wrong on the first pass. **D0 closed the same day, not as a decision but as a record**: the geometry work shipped as one minor and D5's shadow family was never in it, which is the split D0's own text had offered as the alternative. Open: D5. |
| 21.12.0 | **released (2026-09-10)** | The colour and elevation release, and every item in it was found by a check rather than by eye. **`npm run check:oklch`** is the perceptual half of the palette gate: three rules in OKLCH for what a luminance ratio cannot see — a state step below the just-noticeable difference, a status colour outside a chroma band, two statuses that differ by neither 15° of hue nor 0.10 of L. Every threshold was measured across all eleven palettes _before_ being set, and the one rule `docs/backlog.md` had asked for did not survive that: "ramps must be monotonic in L" fails eight of the eleven and eight of them are right, because on a light ground the hover fill is deliberately darker. It found two defects on its first run — `one-light`'s press step and `terminal`'s info and success sitting 4.6° apart. **`--gog-control-boundary-color`** splits the edge that _identifies_ a control from `--gog-border-color`, which draws dividers and is deliberately faint: `gog-chip`, `gog-toggle` and `gog-button-toggle` had been reading the decorative one and measured 1.18 to 2.17:1 against the 3:1 SC 1.4.11 requires. Its eleven values are computed by **`npm run suggest:color`**, which walks lightness in OKLCH holding hue and chroma, so a check can now name the value that would pass. Eight focus indicators that were a pale wash at 1.38:1 read the accent. **And the elevation ladder** — `--gog-elevation-0` … `-5` generated from ten knobs, Z doubling 0/1/2/4/8/16, blur at 3Z because the 26 shadow layers the library already shipped had a median ratio of exactly 3.00 against the rule's 2Z. Before it, eleven themes hand-authored 22 shadow values with no relationship to each other, and two gave a modal dialog the elevation of a card. `gog-menu` was the one component whose height _changed_: it aliased `--gog-dialog-shadow`, so a dropdown menu carried a modal's shadow while the four dropdown panels beside it carried a panel's — four steps apart, and the alias is why nobody saw it. Plus `--gog-select-panel-radius`/`--gog-multiselect-panel-radius` and the select's panel interior. `check:contrast` is 3883 pairs.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 21.13.0 | **released (2026-09-13)** | The collections and entry-points release. **`virtualize` on all four collection components**, each off by default and never switched on at a row count: the three dropdowns share a fixed-pitch window (10 000 options: 21ms against 512ms eager), and `gog-table` needed a second one, because a table row's height cannot be pinned — the window measures rows as they render. **`gog-alert`**, a persistent in-flow message whose `dismissed` means pressed and whose `live` defaults from the severity. **The first secondary entry points**: `@guildofgleks/ui/shared` (internal) and `/table`, `/datepicker`, `/dialog` as thin re-exports, with the root's copies deprecated for 21.14.0 — measured first on a real CLI app, where a lazy route holding the four heavy components produced a 442-byte chunk and left 40 kB in the initial bundle. Plus `check:tokens` rules J and K (K removed five tokens nothing read), `check:glyph-box`, `check:layering`, the toast's severity colours and the button's focus ring.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 21.14.0 | **released (2026-09-13)** | Phase 2 of the entry points: the table, datepicker and dialog exported only from `@guildofgleks/ui/table`, `/datepicker` and `/dialog`, 25 root exports and three helpers removed, three renamed tokens' old names gone, `GOG_DEPRECATIONS` back to `[]`. The split moved the three units' own code out of an app's initial bundle (101.2 → 88.2 kB for a lazy route holding all four) and not their 27 kB of root dependencies. Also `npm run check:install`, the consumer install check as a script, and the rule for when a release needs it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 22.x    | when Angular 22 lands     | the branch split — see `docs/branching-and-support.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |

**The lab is caught up through 21.14.0** (2026-09-13), and `docs/lab-after-publish.md` holds only
21.15.0's section, waiting for that publish. The 21.13.0 pass is described below.
That pass covered: the three entry points in every page's code samples and in the lab's own
imports; `virtualize` documented on the Select, Multiselect, Autocomplete and Table pages with live
demos, and in the Global Config table; a new Alert page; the FAQ (which said nothing virtualizes,
that the package has one entry point, and that nothing is deprecated — all three false after
21.13.0); the comparison page's counts and a code-splitting section; the token reference, **every
row checked against the installed package's declarations** (four removed or renamed tokens the
checklist had not listed); `theme-starter.css`. **Read the changelog as well as the checklist**:
three 21.13.0 items reached the site only because the changelog was read against it, and the
...
Regenerating that mirror surfaced something worth keeping: **21.12.0 turned five of its
hand-written shadow literals into dead code.** `--gog-panel-shadow` and friends used to be
literals in theme.css's palette layer, so the generated block did not carry them and the
hand-written head's copies won; they are derived now, the generated block comes later at equal
specificity, and the head's versions silently stopped doing anything. A release can turn a working
override into an inert one, and only a regeneration notices.

**The `since` chip tail is closed, and it was closed by deriving rather than by counting.** The
`app-since` component computes "is this the latest line" from the installed package and was always
right; its markdown twin — the hand-written `<span class="since">` in `public/docs/*.md` — needed
the `--latest` class typed in, went stale every release, and once the stale ones were removed
could not be filled _at all_. `markdown-renderer.ts` now adds the class on the way out, using an
`isLatestVersion` shared with the component, so a `.md` author writes the version and nothing else.
The three raw spans that were left in Angular templates became `<app-since>`. Verified against the
built site: 21.12.0's chips render filled, older ones outlined.
...
them: the token reference is hand-maintained and says so, and a component whose _look_ changes
...
- `docs/component-geometry.md` — **the ruleset for computed geometry, and the record of the sweep
  ...
  - **All five laws are gated** by `npm run check:geometry`, a CI step, as of 2026-09-09 — laws
    1, 2, 3, 4 and 5, plus **L7** (icon centring, via a second script over the registry's path
    data) and D7 (the four overlay caps that cap themselves against the viewport, via a third).

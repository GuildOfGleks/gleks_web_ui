...
`docs/lab-examples-refactor.md` records how the lab's examples work now: **one compiled file per
example** under `src/app/examples/<component>/`, rendered live *and* shown as source by
`<app-demo>`, with the source text produced by `scripts/generate-example-sources.mjs`
(`npm run check:examples` in CI-style verification, regenerated by every `build:lab`). All 30
component pages and 216 examples are converted, and every example opens in StackBlitz from that
same file. Adding an example = adding one file + one `<app-demo>` tag.

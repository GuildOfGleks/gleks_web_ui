...
| **21.7.0** | **in progress — iterations 0, 1, 2 and 3 of 6 done (2026-08-28)** | **the three deprecated token prefixes are out of the library and `ui-showcase`** — `--gog-btn-*` → `--gog-button-*`, `--gog-ms-*` → `--gog-multiselect-*`, `--gog-confirm-*` → `--gog-confirmation-dialog-*`; `check:tokens` reports zero left, generated artifacts regenerated (`GOG_DEPRECATIONS` now `[]`), verified live in `ui-showcase` under all three custom themes. `check:deprecations` now actually enforces this for the future: it reads `DEPRECATED_NAMESPACES` and fails once a namespace's `removedIn` is at or below the library's version and the stylesheets still contain it — sanity-checked by simulating the overdue case against scratch copies, never a commit. See `docs/token-prefix-removal.md` for the per-file record of what changed. **Still open:** iteration 4 — README.md/AGENTS.md still describe the prefixes as live; the CHANGELOG has a `### Removed` migration table and a `### Fixed` entry from iteration 0, but the two published docs haven't been touched yet. Announced in 21.5.0 with a dated promise, so none of this is optional or deferrable. |

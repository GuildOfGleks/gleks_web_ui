...
| **21.7.0** | **in progress — iterations 0, 1 and 3 of 6 done (2026-08-28)** | **the three deprecated token prefixes are out of the library and `ui-showcase`** — `--gog-btn-*` → `--gog-button-*`, `--gog-ms-*` → `--gog-multiselect-*`, `--gog-confirm-*` → `--gog-confirmation-dialog-*`; `check:tokens` reports zero left, generated artifacts regenerated (`GOG_DEPRECATIONS` now `[]`), verified live in `ui-showcase` under all three custom themes. See `docs/token-prefix-removal.md` for the per-file record of what changed. **Still open:** iteration 2 (`check:deprecations` still doesn't cover CSS — nothing currently enforces this removal happening on schedule for a *future* one) and iteration 4 (README.md/AGENTS.md still describe the prefixes as live; the CHANGELOG has a `### Fixed` entry from iteration 0 but no `### Removed` migration note yet). Announced in 21.5.0 with a dated promise, so none of this is optional or deferrable. |

...
**Latest release: 21.11.0 (2026-09-09), on npm, tagged, and installed at the root** —
`package-lock.json` resolves `@guildofgleks/ui@21.11.0`, so `gleks-ui-lab` builds against it.
...
**One consequence to know before running the gate: `npm run check:theme-starter` is red on
`master` and correctly so.** It compares `theme.css` from the *installed* package against the
lab's hand-maintained `theme-starter.css`, so a published release makes it fail until the lab
catches up. It is the only red check; every other one passes (verified 2026-09-09, after the
publish).
...
What is left is the mirror image, and it is much quieter: **15 raw `<span class="since">` chips
(recounted 2026-09-09; two files) in `projects/gleks-ui-lab/public/docs/*.md` can never be filled
at all**, latest or not. Nobody sees a

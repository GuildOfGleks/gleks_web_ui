...
| `dropdown`                                                             | `appendToBody`, `direction`, `filter`, `filterPosition`, `virtualize` <span class="since" title="Added in 21.13.0">21.13.0</span>                                                                         | `gog-select` and `gog-multiselect`. `gog-datepicker` and `gog-autocomplete` also honour `appendToBody` and `direction` for their panels, but not `filter`/`filterPosition` (neither has a separate search box). `virtualize` reaches the three dropdowns — select, multiselect and autocomplete — and not `gog-table`, whose own `virtualize` has no app-wide key.                                                                                                                                                                             |
...
| `scroll`                                                               | `autoHide`, `hideDelay`, `size`, `overscrollBehavior`, `showTrack` <span class="since" title="Added in 21.3.1">21.3.1</span>, `horizontalWheel` <span class="since" title="Added in 21.9.0">21.9.0</span> | `gog-scroll`, and every component that uses one internally.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
